...
```javascript
const token =6122929150:AAERJZ8Q7Ytackq4NVxW_dCXJ8At_Ut0OaQ' ;
const bot = new TelegramBot(token, { polling: true }); 
bot.onText(/\/start/, (msg) => {
  const chatId = msg.chat.id;

  bot.sendMessage(chatId, "يرجى الاشتراك في القناة للاستمرار في استخدام البوت");
  bot.sendMessage(chatId, "https://t.me/sms_voip_otpbot", { disable_web_page_preview: true });

  // اذا لم يتم الاشتراك يتم ايقاف البوت 
  bot.on("channel_post", (message) => {
    if (message.chat.id == -100xxxxxxxxxx && message.new_chat_members.some((member) => member.id === msg.from.id)) {
      bot.sendMessage(chatId, `شكراً لك على الاشتراك في القناة`);
    } else {
      bot.sendMessage(chatId, `للاسف لم تقم بالاشتراك في القناة بعد`);
      bot.stopPolling();
    }
  });
});
